#!/usr/bin/env node
/**
 * BC AUTO XPERIENCE — NocoDB Schema Patch v5
 * ═══════════════════════════════════════════
 * Adds `branch_id` to missing tables to support correct branch scoping and prevent cross-branch data leaks.
 * Idempotent: skips columns that already exist.
 *
 * New columns:
 *   job_items: branch_id (SingleLineText)
 *   document_items: branch_id (SingleLineText)
 *   hr_attendance: branch_id (SingleLineText)
 *   hr_leaves: branch_id (SingleLineText)
 *
 * Usage:
 *   node scripts/patch-schema-v5.mjs \
...
if (!EMAIL || !PASSWORD) {
    console.error('❌ Usage: node patch-schema-v5.mjs --url=... --email=... --password=...')
    process.exit(1)
}
...
const PATCHES = [
    {
        table: 'job_items',
        columns: [{ title: 'branch_id', uidt: 'SingleLineText' }]
    },
    {
...
async function main() {
    console.log('═══════════════════════════════════════════')
    console.log('  NocoDB Schema Patch v5 — Branch Scoping Hardening')
    console.log('═══════════════════════════════════════════')
