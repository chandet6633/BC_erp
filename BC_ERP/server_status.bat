...
echo  ─── HEALTH CHECKS ──────────────────────────────────────────
echo.
for %%c in (bc-management bc-mungkhud uvgard-app) do (
...
echo   [2] View logs (bc-management)
...
if "%choice%"=="1" (
    ...
    docker restart bc-management bc-mungkhud uvgard-app
    timeout /t 5 /nobreak >nul
    goto menu
)
if "%choice%"=="2" (
    echo.
    docker logs --tail 30 bc-management
    echo.
    pause
    goto menu
)
