...
window.DataManager = {
    ...
    async syncLeaves(monthStr, depts = [], onProgress = null) {
        ...
        console.log(`[SyncLeaves] Fetching all leaves for client-side filtering...`);
        // Fetch ALL (or recent) leaves to avoid 400 Bad Request on ambiguous schema
        // REMOVED SORT: It caused 400 Bad Request if field is not indexed/text.
        const allLeavesRecords = await pb.collection('hr_leaves').getFullList();

        // Filter in JS
        const existingLeaves = allLeavesRecords.filter(r => {
            // Logic: Overlap
            return r.start_date <= end && r.end_date >= start;
        });
        console.log(`[SyncLeaves] Client filtered ${existingLeaves.length} matching leaves.`);
    ...
    async getLeaves(monthStr, dept = null) {
        ...
        try {
            ...
            const allLeaves = await pb.collection('hr_leaves').getFullList();

            return allLeaves
                .filter(r => {
                    const matchDate = r.start_date <= end && r.end_date >= start;
                    const matchDept = !dept || dept === '' ? true : r.department === dept;
