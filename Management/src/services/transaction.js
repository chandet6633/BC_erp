...
export const TransactionService = {
    ...
    async importProductGroups(rows, branch, reportMonth) {
        ...
        if (toUpsert.length > 0) {
            ...
            if (reportMonth) {
                const bFilter = (branch === 'BC Auto Service' || branch === 'main' || !branch)
                    ? "(branch='BC Auto Service' || branch='main' || branch='')"
                    : `branch='${branch}'`;
    ...
    async getDuplicates() {
        ...
        const txByUniqueKey = {};
        allTx.forEach(tx => {
            const branch = tx.branch || 'BC Auto Service';
        ...
        const siByKey = {};
        allSI.forEach(si => {
            const branch = si.branch || 'BC Auto Service';
