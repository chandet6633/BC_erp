...
MungkhudShop is a **Thai-language automotive service shop ERP** built with Vite + Vanilla JS + PocketBase. It manages jobs (car repairs), inventory (parts), purchasing (documents), and reports. The UI is entirely in Thai with a Glassmorphism design theme (Navy/Gold/White).

> [!CAUTION]
> **Do NOT:**
> - Install React, Vue, Tailwind, or any framework
> - Change PocketBase URL without updating `pb.js`, `setup-db.js`, `setup-auth.js`, `start-mungkhud.bat`
...
| PB Admin | `http://127.0.0.1:8091/_/` |
...
| `src/services/pb.js` | **Data layer.** 6 CRUD functions + audit logging + Management PB connection. (`pb.js:62-110`) |
...
| `scripts/setup-db.js` | **Schema.** Creates all PocketBase collections. Source of truth for DB. |
...
PocketBase auto-generates REST APIs from collections. Use these functions from `pb.js`:
...
**NOT** using PocketBase built-in auth. Custom `app_users` collection.
