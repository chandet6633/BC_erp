...
> **Migration note (current runtime):** MungkhudShop no longer talks directly to PocketBase. The live code uses `src/services/pb.js` as a shim over `@shared/nocodb-adapter.js`, so data flows through `/api/auth` and `/api/data` on the Express API, then into NocoDB. Older PocketBase references below are migration history unless they are explicitly reflected in current source.
...
| Attribute | Value |
|-----------|-------|
| **Stack** | Vite 6 + Vanilla JS + Express API + NocoDB |
...
| **Database** | NocoDB through `/api/*` (xc-token stays server-side) |
