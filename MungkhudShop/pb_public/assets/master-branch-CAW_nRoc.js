import{n as m,s,u as y,d as f,f as q,r as S,m as g,h as x}from"./index-9Z-6SckO.js";function k(r){let n=[],t=null;r.innerHTML=`
    ...
    `;async function d(){const l=b.querySelector("#branchGrid");l.innerHTML='<div style="padding:var(--sp-8);text-align:center;color:var(--color-text-muted);">กำลังโหลด...</div>',n=await q("branches",{}),l.innerHTML=S({columns:[{key:"code",label:"รหัส"},{key:"name",label:"ชื่อสาขา"},{key:"address",label:"ที่อยู่"},{key:"phone",label:"โทร"},{key:"is_active",label:"สถานะ",render:a=>a.is_active!==!1?'<span class="badge badge-open">เปิด</span>':'<span class="badge badge-cancelled">ปิด</span>'},{key:"actions",label:"",render:a=>`
                ...
                `}],items:n}),l.querySelectorAll(".btn-edit").forEach(a=>a.onclick=()=>v(a.dataset.id)),l.querySelectorAll(".btn-delete").forEach(a=>a.onclick=()=>h(a.dataset.id))}function v(l){const a=n.find(c=>c.id===l);a&&(t=l,e.querySelector("#branchCode").value=a.code||"",e.querySelector("#branchName").value=a.name||"",e.querySelector("#branchAddress").value=a.address||"",e.querySelector("#branchPhone").value=a.phone||"",e.querySelector("#branchEmail").value=a.email||"",e.querySelector("#branchTaxId").value=a.tax_id||"",e.querySelector("#branchActive").value=a.is_active!==!1?"true":"false",r.querySelector('.tab-btn[data-tab="add"]').click())}async function h(l){await g("ยืนยันลบ","คุณต้องการลบสาขานี้?")&&(await x("branches",l),s("ลบสาขาเรียบร้อย","success"),d())}e.querySelector("#btnSaveBranch").addEventListener("click",async()=>{const l=e.querySelector("#branchCode").value.trim(),a=e.querySelector("#branchName").value.trim();if(!l||!a)return s("กรุณากรอกรหัสและชื่อสาขา","error");const c={code:l,name:a,address:e.querySelector("#branchAddress").value,phone:e.querySelector("#branchPhone").value,email:e.querySelector("#branchEmail").value,tax_id:e.querySelector("#branchTaxId").value,is_active:e.querySelector("#branchActive").value==="true"};try{t?await y("branches",t,c):await f("branches",c),s("บันทึกสาขาเรียบร้อย","success"),u(),r.querySelector('.tab-btn[data-tab="search"]').click(),d()}catch(p){s("เกิดข้อผิดพลาด: "+p.message,"error")}});function u(){t=null,e.querySelector("#branchCode").value="",e.querySelector("#branchName").value="",e.querySelector("#branchAddress").value="",e.querySelector("#branchPhone").value="",e.querySelector("#branchEmail").value="",e.querySelector("#branchTaxId").value="",e.querySelector("#branchActive").value="true"}e.querySelector("#btnClearBranch").addEventListener("click",u),d()}export{k as initBranchPage};
