...
async function loadDashboardData(container, forceRefresh = false) {
    ...
    try {
        ...
        // Open jobs count
        const openJobs = await fetchFullList('jobs', { filter: `status='open'${bf}` })

        // Today's closed jobs (revenue)
        const todayJobs = await fetchFullList('jobs', {
            filter: `status='closed' && end_date >= '${today}'${bf}`
        })

        // This month's closed jobs
        const monthJobs = await fetchFullList('jobs', {
            filter: `status='closed' && end_date >= '${monthStart}'${bf}`
        })

        // Last 7 days jobs for sparklines
        const recentJobs = await fetchFullList('jobs', {
            filter: `start_date >= '${last7Start}'${bf}`
        })

        // Customers count
        const customers = await fetchFullList('customers')

        // Products + stock for low-stock check
        const products = await fetchFullList('products')
        const ledgers = await fetchFullList('stock_ledgers')
        ...
        container.querySelector('#statJobs').textContent = '!'
    }
}
...
function renderDashboard(container, data) {
    ...
    container.querySelector('#statJobs').textContent = openCount
    container.querySelector('#statTodayRev').textContent = formatCurrency(todayRev)
    container.querySelector('#statRevenue').textContent = formatCurrency(monthRev)
    container.querySelector('#statCustomers').textContent = customerCount

    // Low stock
    const lowStockProducts = products.filter(p => {
        const qty = stockMap[p.id] || 0
        const threshold = p.min_qty || 5
        return qty <= threshold
    })
    container.querySelector('#statLowStock').textContent = lowStockProducts.length
    ...
    if (alertEl) {
        if (lowStockProducts.length > 0) {
            ...
            if (!window.__lowStockNotified) {
                ...
                notifyLowStock(lowStockProducts.map(p => ({ name: p.name, qty: stockMap[p.id] || 0 }))).catch(() => {})
            }
        } else {
    ...
    const jobsByDay = last7.map(day =>
        recentJobs.filter(j => j.created && j.created.split(' ')[0] === day).length
    )
    ...
    if (sortedRecent.length === 0) {
        ...
        jobsEl.innerHTML = renderDataGrid({
            columns: [
                ...
                { key: 'status', label: 'สถานะ', render: r => `<span class="badge badge-${r.status === 'closed' ? 'closed' : r.status === 'cancelled' ? 'cancelled' : 'open'}">${r.status === 'closed' ? 'ปิดงาน' : r.status === 'cancelled' ? 'ยกเลิก' : 'เปิด'}</span>` },
    ...
    if (lowStockProducts.length === 0) {
        ...
        lowEl.innerHTML = renderDataGrid({
            columns: [
                ...
                { key: 'min_qty', label: 'ขั้นต่ำ', render: r => r.min_qty || 5 },
            ],
