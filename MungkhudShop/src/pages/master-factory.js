...
export function createMasterPage(cfg) {
    return function (container) {
        ...
        const gridCols = [...cfg.columns, {
            key: 'actions', label: 'จัดการ', render: (item) => `
                <div class="grid-actions">
                    <button class="btn btn-sm btn-outline btn-edit" data-id="${item.id}" title="แก้ไข"><span class="material-icons-outlined" style="font-size:16px;">edit</span></button>
                    <button class="btn btn-sm btn-danger btn-delete" data-id="${item.id}" title="ลบ"><span class="material-icons-outlined" style="font-size:16px;">delete</span></button>
                </div>
            `
        }]
        ...
        async function loadData() {
            ...
            try {
                ...
                if (currentItems.length === 0) {
                    gridEl.innerHTML = '<div style="padding:var(--sp-8);text-align:center;color:var(--color-text-muted);">ยังไม่มีข้อมูล — เพิ่มรายการแรกได้ที่แท็บ "เพิ่ม / แก้ไข"</div>'
                    return
                }
        ...
        const fieldsHtml = cfg.fields.map(f => {
            ...
            if (f.type === 'textarea') {
                ...
                // Placeholder select — will be populated after render
