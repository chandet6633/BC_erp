...
export function initReportFinancePage(container) {
    ...
    async function runReport() {
        ...
        try {
            ...
            const purchaseDocs = await fetchFullList('documents', {
                filter: `(doc_type='RR' || doc_type='PI' || doc_type='PCN') && issue_date >= '${sanitizeFilter(start)}' && issue_date <= '${sanitizeFilter(end)} 23:59:59'`
            })
            ...
            if (type === 'income_expense') {
                ...
                container.querySelector('#finReportTitle').textContent = 'สรุปยอดขายรายวัน'
                const dayMap = {}
                for (const j of closedJobs) {
                ...
                lastColumns = [
                    { key: 'date', label: 'วันที่' },
                    { key: 'count', label: 'จำนวนใบงาน' },
                    { key: 'total', label: 'ยอดขาย', render: r => formatCurrency(r.total) },
                    { key: 'avg', label: 'เฉลี่ยต่อใบ', render: r => formatCurrency(r.count > 0 ? r.total / r.count : 0) },
                ]
                ...
                statsEl.innerHTML = `
                    ...
                    <div class="stat-card"><div class="stat-icon green"><span class="material-icons-outlined">paid</span></div><div class="stat-value">${formatCurrency(totalIncome)}</div><div class="stat-label">ยอดขายรวม</div></div>
                    <div class="stat-card"><div class="stat-icon orange"><span class="material-icons-outlined">trending_up</span></div><div class="stat-value">${formatCurrency(avgDaily)}</div><div class="stat-label">เฉลี่ยต่อวัน</div></div>
                `
            }
    ...
    container.querySelector('#btnFinToday')?.addEventListener('click', () => {
        container.querySelector('#finStartDate').value = today
        container.querySelector('#finEndDate').value = today
        runReport()
    })
    container.querySelector('#btnFinThisMonth')?.addEventListener('click', () => {
        ...
        container.querySelector('#finStartDate').value = first
        container.querySelector('#finEndDate').value = today
        runReport()
    })
}
