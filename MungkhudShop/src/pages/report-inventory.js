...
export function initReportInventoryPage(container) {
    ...
    async function runReport() {
        ...
        try {
            ...
            for (const l of ledgers) {
                if (!stockMap[l.product_id]) { stockMap[l.product_id] = 0; costMap[l.product_id] = 0 }
                stockMap[l.product_id] += (l.qty || 0)
                costMap[l.product_id] += (l.total_value || 0)
            }
            ...
            if (type === 'stock_valuation') {
                ...
                lastColumns = [
                    ...
                    { key: 'status', label: 'สถานะ', render: r => { const m = r.min_qty || 5; return r.qty <= 0 ? '<span class="badge badge-cancelled">หมด</span>' : r.qty <= m ? '<span class="badge badge-pending">ใกล้หมด</span>' : '<span class="badge badge-closed">ปกติ</span>' } },
                ]
                // FIX #1: use p.id (not p.code) to join with ledger
                // FIX #7: use product.cost (WAC) as primary source
                lastItems = filtered.map(p => {
                    const qty = stockMap[p.id] || 0
                    const cost = p.cost || (qty > 0 ? (costMap[p.id] || 0) / qty : 0)
                ...
                lastColumns = [
                    { key: 'created', label: 'วันที่', render: r => r.created ? r.created.split(' ')[0] : '-' },
                ...
                lastItems = [...ledgers].sort((a, b) => (b.created || '').localeCompare(a.created || ''))
                statsEl.innerHTML = `
                ...
                lastColumns = [
                    ...
                    { key: 'name', label: 'ชื่อสินค้า' },
                    { key: 'qty', label: 'คงเหลือ' },
                    { key: 'status', label: 'สถานะ', render: r => r.qty <= 0 ? '<span class="badge badge-cancelled">หมด</span>' : '<span class="badge badge-pending">ใกล้หมด</span>' },
                ]
                // FIX #1 + #2: use p.id, use min_qty
                lastItems = filtered.map(p => ({ ...p, qty: stockMap[p.id] || 0 })).filter(p => p.qty <= (p.min_qty || 5)).sort((a, b) => a.qty - b.qty)
                statsEl.innerHTML = `
    ...
    function renderInvChart(items) {
        ...
        const normal = items.filter(i => i.qty > 5).length
