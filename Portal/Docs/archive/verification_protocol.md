# BC Auto Portal — Verification Protocol
...
- [ ] **Build Check**: Run `npm run build` in the Portal root.
  ```bash
  cd Portal
  npm run build
...
- [ ] **Container Build**: `docker compose build portal` completes without errors.
- [ ] **Container Start**: `docker compose up -d portal` → container healthy.
