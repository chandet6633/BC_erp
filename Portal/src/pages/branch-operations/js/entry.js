...
import { fetchBranchMetadata, getBranchLabel } from '@shared/branch-metadata.js'
import { getSession, getSessionLanguage, isRealBranchId } from '@shared/session.js'
...
async function initBranchSelector() {
    ...
    if (['admin', 'owner'].includes(user.role)) {
        try {
            const branches = await fetchBranchMetadata({ includeAll: false, force: true });
            const dropdown = document.getElementById('branchDropdown');
            branches.forEach(b => {
                const name = b.branch_id || '';
                if (!name) return;
                const opt = document.createElement('option');
                opt.value = name;
                opt.textContent = getBranchLabel(name, branches, getSessionLanguage());
                dropdown.appendChild(opt);
            });

            // Pre-select current branch if set
            const current = getSession().branchId;
            if (isRealBranchId(current)) {
...
function getSelectedBranch() {
    ...
    return user.branch_id || user.branch || getSession().branchId || '';
}
