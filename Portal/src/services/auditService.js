...
export const AuditService = {
    ...
    runGroupAudit(productGroups, serviceItems, transactions, selectedMonth) {
        ...
        const siForMonth = serviceItems.filter(i => (i.open_date || i.CreatedAt || '').startsWith(selectedMonth));
        const txForMonth = transactions.filter(tx => (tx.open_date || tx.CreatedAt || '').startsWith(selectedMonth));
    ...
    async log(action, details, module = 'system') {
        try {
            ...
            const branch = getSession().branchId || user?.branch_id || user?.branch || '';
