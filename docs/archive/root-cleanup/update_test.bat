...
if not exist "Portal\pb_data" (
    echo [ERROR] Portal\pb_data not found.
...
if not exist "Portal\pb_data_test" mkdir "Portal\pb_data_test"
if not exist "MungkhudShop\pb_data_test" mkdir "MungkhudShop\pb_data_test"

echo       Copying Portal data...
xcopy /E /Y /Q "Portal\pb_data\*" "Portal\pb_data_test\" >nul
...
echo.
echo       Running Portal setup-db (port 9092)...
cd Portal
...
echo   Portal   : http://localhost:9092
